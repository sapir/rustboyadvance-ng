    ...
    pub fn get_cycles(
        ...
        match width {
            MemoryAccess8 | MemoryAccess16 => match access {
                NonSeq => self.cycle_luts.n_cycles16[(addr >> 24) as usize],
                Seq => self.cycle_luts.s_cycles16[(addr >> 24) as usize],
            },
            MemoryAccess32 => match access {
                NonSeq => self.cycle_luts.n_cycles32[(addr >> 24) as usize],
                Seq => self.cycle_luts.s_cycles32[(addr >> 24) as usize],
            },
        }
    }
}
