...
struct EepromChip<'a, M>
where
    M: BackupMemoryInterface<'de>,
{
    memory: M,
...
impl<'a, M> EepromChip<'a, M>
where
    M: BackupMemoryInterface<'a>,
{
    fn new(memory: M) -> EepromChip<'a, M> {
        EepromChip {
            memory: memory,
...
#[derive(Serialize, Deserialize, Debug)]
pub struct SpiController<'a, M>
where
    M: BackupMemoryInterface<'a>,
{
    chip: RefCell<EepromChip<'a, M>>,
}

impl<'a, M> SpiController<'a, M>
where
    M: BackupMemoryInterface<'a>,
{
    pub fn new(m: M) -> SpiController<'a, M> {
        SpiController {
